...
async function enter(){
  if(!HAIFU) HAIFU=await (await fetch('data/haifu.json?v=20260527o')).json();
  if(!GRAPHS){ GRAPHS=(await (await fetch('data/dashboard.json?v=20260527o')).json())['施設']; buildGraphIndex(); }
  if(!MULTILINE){ try{ MULTILINE=(await (await fetch('data/multiline_series.json?v=20260527o')).json())['施設']||{}; }catch(e){ MULTILINE={}; } }
  show('dash');
