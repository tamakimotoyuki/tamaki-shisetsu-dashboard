...
async function enter(){
  if(!HAIFU) HAIFU=await (await fetch('data/haifu.json?v=auto-20260529-1712')).json();
  if(!GRAPHS){ GRAPHS=(await (await fetch('data/dashboard.json?v=auto-20260529-1712')).json())['施設']; buildGraphIndex(); }
  if(!MULTILINE){ try{ MULTILINE=(await (await fetch('data/multiline_series.json?v=auto-20260529-1712')).json())['施設']||{}; }catch(e){ MULTILINE={}; } }
  show('dash');
...
function printItemRow(it){
  ...
  const lv=(it['レベル']===2 || /^うち/.test(nm))?' lv2':'';  // レベル2 or「うち〜」＝上位項目の内数（アンギオ室のPICC/CVポート、退院のうち死亡、緊急入院のうち救急搬送 等）。合計に足さない＝字下げで区別
...
function printHandout(){
  ...
  PRINT_PAGES.forEach((facs,pi)=>{
    ...
    root.appendChild(hd);
    const grp=document.createElement('div'); grp.className='pgroup';   // ★ページ単位の左右2列。施設/部署がこの中を流れて両列を埋める
    facs.forEach(hint=>{
      const fac=resolveFac(hint); if(!fac||!HAIFU[fac]) return;
      const depKeys=Object.keys(HAIFU[fac]);
      const multi=depKeys.length>1;
      const block=document.createElement('div');
      // 複数部署の施設は列・ページをまたいで流す（部署単位で塊を保つ）。単一部署の小施設は丸ごと保持。
      block.className='pfac'+(multi?' flow':'');
